...
export function HomePage() {
  ...
  const notOkCount = recentChecks.filter(check => check.statut === 'NOT_OK').length;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Checks récents</h3>
          <p className="text-3xl font-bold text-gray-600">{recentChecks.length}</p>
        </div>
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Statut OK</h3>
          <p className="text-3xl font-bold text-green-600">{okCount}</p>
        </div>
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Problèmes</h3>
          ...
          {applications.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {applications.map((app) => (
                ...
                >
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">{app.nom}</h3>
                  {app.description && (
      ...
      {recentChecks.length > 0 && (
        <div className="bg-white rounded-lg shadow-md">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">Sanity Checks récents</h2>
          </div>
          <div className="p-6">
              ...
              {recentChecks.map((check) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    {check.commentaire && (
                      <p className="text-sm text-gray-500 mt-1">{check.commentaire}</p>
                    )}
                  </div>
